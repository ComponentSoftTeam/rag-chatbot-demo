...
project_name = "Component Soft init_chroma_db"
...
@traceable(run_type="retriever")
def parse_txt(path: str, txt_name: str, page_offset: int = 0):
    ...
    with open(path, 'r') as f:
        for i, line in enumerate(f):
            ...
            txt_texts += page_text
            page_sum += len(page_text)
            page_characters += page_sum

    # Filter the empty strings
    txt_texts = [text for text in txt_texts if text]
    ...
    already_read_characters = 0
    curr_page = 0
    for chunk in character_splitter.split_text(''.join(txt_texts)):
        curr_page = i + 1
        """already_read_characters += len(chunk)
        for i, page in enumerate(page_characters):
            if already_read_characters < page:
                curr_page = i -1
                break"""
        docs.append(Document(page_content=chunk, metadata={"source_file": str(txt_name), "page_num": curr_page+page_offset}))
...
def main():
    ...
    path_book1_txt = "./data/S-Sedan Owners Manual.txt"
    path_book2 = "./data/Electrical Drives_new.pdf"
    path_book3 = "./data/MIT_6.685_Electric_Machines.pdf"
